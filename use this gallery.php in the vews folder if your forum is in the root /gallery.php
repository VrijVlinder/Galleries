<?php if (!defined('APPLICATION')) exit();
	...
	echo '<div class="Tabs"><a href="gallery" title="Main Gallery"><h19>'.T('Picture Galleries').'</h19></a></div>';
	$dir = 'uploads/picgal/';
	...
	switch($show){
			...
			foreach($folders as $vv){
				...
				if(is_file($dir.$vv.'/notes.dat.php')){
					...
					for($i=0;$i<100;$i++){
						...
						if(isset($fi[$i])){
							$pg = explode('|', $fi[$i]);
							echo '<div class="pics"><a class="fancybox" target="_blank" rel="group" href="/'.$dir.$folder.$title.'/'.$pg[0].'" title="'.stripslashes(htmlentities(urldecode($pg2[1]))).'"><img src="/'.$dir.$vv.'/'.$pg[0].'" alt="User Images"/></a></div>';
						}
		...
		case 'gallery':
			...
			for($i=1; $i<$cc;$i++){
				...
				$bb++;
				echo '<div class="pics"><img src="/'.$dir.$folder.'/'.$pg2[0].'" alt="UserImages" /><br /></div>';
			}
...
if($UserHasPermission) {
	$dir = 'uploads/picgal/';
	$PATH2IM = '';
	// with a trailing slash -- /usr/local/bin/ for liquidweb.com servers.
	// making a new folder..
...
if($UserHasPermission) {
	...
	if(isset($_POST['PGUpload'])){
		...
		if(isset($theFile['small'])){
			...
			if(is_array($gogo)) echo $gogo[0]; else echo '<b><h3>â�¥. Â¸. â�¢**â�¢. Â¸â�¥Â¸. â�¢**â�¢. Â¸â�¥.Success File Has Been Uploaded.â�¥. Â¸. â�¢**â�¢. Â¸â�¥Â¸. â�¢**â�¢. Â¸â�¥ </h3></b><br /><a class="Button" href="gallery" title="Go Back To Main Gallery">Go Back to Main Gallery</a><br />';
		} else {
...
if($UserHasPermission) {
	...
	if(isset($_POST['imgName']) && isset($_POST['imgSave'])) {
		...
		switch($_POST['imgSave']) {
			case 'Delete Image':
				echo 'deleting the image...<br /><a class="Button" href="gallery" title="Go Back To Main Gallery">Go Back to main Gallery</a><br/>';
				...
				break;
			case 'Save Image';
			echo 'saving the caption on the image..<br /><a class="Button" href="/gallery" title="Go Back To Main Gallery">Go Back to main Gallery</a><br />';
			$write2file = new FileWriting;
...
<div class="Info">
		...
		<form action="'. $_SERVER['REQUEST_URI'].'" method="post">
			<div class="picbox">
			<img src="/'.$dir.$folder.'/'.$pg2[0].'" alt="image" /><br />
...
<div class="Info">
	...
	<form action="<?php  echo $_SERVER['REQUEST_URI']; ?>" method="post">
		<div>Create new Album, Please Pick a Name  - No Spaces</br> <input class="InputBox" stype="text" size="20" name="PGNewFolder" /></div>
...
function uploadNresizeGallery($file, $dir){
	...
	if($countinue == true){
		...
		if($file['image']['error']){
			$errors[] = "Sorry nothing was uploaded..or";
			echo '<h21>Sorry nothing was uploaded</h21>';
		}
		...
		for(;;
			...
			if(!file_exists($dir.'image'.$nr.$ext))break;
		}
	...
	$errors[] = "something happened to the directories permission";
}
...
private
function write2File($file, $lines, $mode){
	$fp = fopen($file, $mode);
	flock($fp, 2);
	fwrite($fp, $lines);
	flock($fp, 3);
