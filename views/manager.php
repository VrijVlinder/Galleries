...
$this->Head->Title('Image Upload Manager');

$this->AddCssFile('custom.css');
$this->AddCssFile('gallery.css', 'plugins/PictureGallery');


echo '<h19>Picture Gallery Manager</h19></br><br>';

echo'<h12>SUCCESS YOUR IMAGE WAS UPLOADED!!</h12><br><br>';

echo' <a class="gal" href="/forum/gallery">♥¸. •**•. ¸GoBack2Gallery¸. •**•. ¸♥</a>';
...
// couldnt upload the picture.
		...
		$pg = 'error';
		echo 'Error,something went horribly bad :( ';
		print_r($theFile);
	}
}
...
// browse the pictures..
		...
		<form action="'. $_SERVER['REQUEST_URI'].'" method="post">
			<div class="picbox">
			<img src="/forum/'.$dir.$folder.'/'.$pg2[0].'" alt="picture" /><br />
	...
	break;
	case 'error':
		echo '<b>Name already exists, please pick another name.</b>';
	break;
	default:
...
function uploadNresizeGallery($file, $dir){
	...
	$countinue = true;
	if(!is_dir($dir)){
		return 'Cannot save because the folder was not found';
	}

	if($countinue == true){
		if($file['image']['error']){ $errors[] = "nothing was uploaded..sorry"; echo '<pre>'; print_r($file['image']); echo '</pre>'; }
